...
<div class="space-y-4">
  ...
  <div class="flex items-center gap-3 flex-wrap">
    ...
    <div class="flex items-center gap-2">
      <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-lg bg-gray-100 text-gray-600">
        <span class="w-1.5 h-1.5 rounded-full bg-gray-400"></span>
        {pendingTasks.length} {t.pending || '待执行'}
      </span>
      <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-lg bg-emerald-50 text-emerald-700">
        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
        {completedTasks.length} {t.completed || '已完成'}
      </span>
      {#if failedTasks.length > 0}
        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-lg bg-red-50 text-red-700">
          <span class="w-1.5 h-1.5 rounded-full bg-red-500"></span>
          {failedTasks.length} {t.failed || '失败'}
        </span>
      {/if}
      {#if cancelledTasks.length > 0}
        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-lg bg-gray-50 text-gray-500">
          <span class="w-1.5 h-1.5 rounded-full bg-gray-300"></span>
          {cancelledTasks.length} {t.cancelled || '已取消'}
        </span>
      {/if}
    </div>
  ...
  <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
    ...
    {:else if pendingTasks.length === 0}
      <div class="px-4 py-10 text-center">
        ...
        <p class="text-[12px] text-gray-400 mb-3">{t.noScheduledTasks || '暂无待执行的定时任务'}</p>
        <button
          class="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-medium text-white bg-gray-900 hover:bg-gray-800 rounded-lg transition-colors cursor-pointer"
          onclick={openCreateForm}
        >
    ...
    {:else}
      <div class="p-3 space-y-2">
        {#each pendingTasks as task (task.id)}
          <div class="flex rounded-lg border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all overflow-hidden">
            <div class="w-1 flex-shrink-0 {task.status === 'executing' ? 'bg-amber-400' : 'bg-blue-400'}"></div>
                  ...
                  <div class="flex items-center gap-3 text-[11px] text-gray-500">
                    ...
                    {#if task.status === 'pending'}
                      <span class="flex items-center gap-1 text-blue-600 font-medium">
                        ...
                        {getTimeRemaining(task.scheduledAt)}
                      </span>
                    {/if}
              ...
              <div class="space-y-3">
                {#each historyTasks as task (task.id)}
                  <div class="relative flex gap-3">
                    <!-- Timeline dot -->
                    <div class="absolute -left-5 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-white flex items-center justify-center {task.status === 'completed' ? 'bg-emerald-400' : task.status === 'failed' ? 'bg-red-400' : 'bg-gray-300'}">
                    </div>
                    <div class="flex-1 min-w-0 pb-1">
                      <div class="flex items-center justify-between">
                        ...
                        {#if task.taskResult}
                          <button
                            class="px-2 py-1 text-[10px] font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors cursor-pointer flex-shrink-0"
