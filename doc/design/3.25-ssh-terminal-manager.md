...
// 每个 session 对象
{
  ...
  error: '',
  host, user,                    // 显示用
  isExternal: false,             // 是否为外部服务器连接
  extPort: 22,                   // 外部服务器端口（仅 isExternal 时有效）
  extPassword: '',               // 外部服务器密码（仅 isExternal 时有效）
  extKeyPath: '',                // 外部服务器密钥路径（仅 isExternal 时有效）
}
```
...
│       └─ 多实例 → 显示实例选择器                         │
│                   ├─ 点击单个实例 → createSession(...)    │
│                   └─ 点击「全部连接」→ 循环 createSession │
│       ↓                                                  │
│   也可点击「手动输入 ID」→ caseId + displayName 表单     │
└──────────────────────────────────────────────────────────┘
    ↓
┌─ 选择「外部服务器」─────────────────────────────────────┐
│   填写：主机地址、端口(默认22)、用户名(默认root)         │
│   选择认证方式：[密码] / [密钥]                          │
│       ├─ 密码 → 输入密码                                 │
│       └─ 密钥 → 输入密钥文件路径                         │
│       ↓                                                  │
│   handleExternalSessionSubmit()                          │
│       ↓                                                  │
│   创建 session { isExternal: true, caseId: null, ... }   │
│       ↓                                                  │
│   connectSession() 检测 isExternal                       │
│       ├─ true  → StartSSHTerminalDirect(host, port, ...) │
│       └─ false → StartSSHTerminalInstance(caseId, ...)   │
└──────────────────────────────────────────────────────────┘
```

### 外部服务器连接

外部服务器连接不依赖任何 RedC 场景或部署，直接通过 `StartSSHTerminalDirect` 连接任意 SSH 服务器。

**后端实现** (`app_ssh.go`)：

```go
func (a *App) StartSSHTerminalDirect(host string, port int, user, password, keyPath string, rows, cols int) (string, error) {
    // 直接构造 SSHConfig，不查询 Case
    sshConfig := &sshutil.SSHConfig{
        Host: host, Port: port, User: user,
        Password: password, KeyPath: keyPath,
        Timeout: 15 * time.Second,
    }
    // sessionID 格式: "ext-{host}-{port}-{timestamp}"
    // 与 Case 的 "{caseID}-{timestamp}" 区分
}
```

**前端实现** (`SSHManager.svelte`)：

- `dialogMode` 状态控制对话框显示 `'cases'` 或 `'external'`
- 外部服务器的 session 对象 `caseId` 为 `null`，`isExternal` 为 `true`
- `connectSession()` 根据 `session.isExternal` 决定调用哪个后端 API
- 认证方式切换使用与模式选择器相同的 pill 风格 UI

**使用场景：**
- 本地虚拟机（VMware/VirtualBox/Parallels）
- 局域网内的物理服务器或容器
- 云厂商直接购买的非 RedC 管理的服务器
- 跳板机/堡垒主机
...
6. **外部服务器会话的 `caseId` 为 `null`**：依赖此字段判断是否为 RedC 场景的代码需做空值检查
7. **外部服务器 sessionID 前缀为 `ext-`**：可用于区分日志和调试
