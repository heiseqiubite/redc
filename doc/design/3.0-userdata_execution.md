...
`SSHManager.svelte`（位于 `frontend/src/components/SSH/`）是独立的全页面 SSH 终端管理器，提供多会话标签页、命令片段面板、文件管理等高级功能。详见 [3.25-ssh-terminal-manager.md](3.25-ssh-terminal-manager.md)。

本文档描述的 `SSHModal.svelte` 和 `WebTerminal.svelte` **仍然存在且在使用中**，它们位于 `frontend/src/components/Cases/` 目录下，专门服务于"预定义场景"页面的快速运维弹窗场景。两者职责不同：

| 组件 | 位置 | 用途 |
|------|------|------|
| `SSHModal.svelte` | `Cases/` | 预定义场景的运维弹窗（含 Userdata 执行） |
| `WebTerminal.svelte` | `Cases/` | 预定义场景的内嵌 Web 终端 |
| `SSHManager.svelte` | `SSH/` | 独立全页面 SSH 终端管理器（多会话、外部服务器） |
