...
| 预定义模板 | 模板名称包含 `/aliyun/` 或 `/aws/` 或其他云厂商目录 |
...
- 其他目录（aliyun, aws, tencent, volcengine, vultr 等）- 预定义模板

---

## 4. 更新记录

### 2025-02-25 更新：右键菜单配置持久化

#### 问题描述
右键菜单禁用设置在页面刷新后会丢失，无法持久化保存。

#### 解决方案
新增 GUI 配置文件 `~/redc/gui_settings.json`，用于保存 GUI 相关设置。
...
var LoadedGUISettings *GUISettings = nil
...
- `SetDisableRightClick`: 设置时保存到 GUI 配置文件
- `GetDisableRightClick`: 从 GUI 配置文件读取
- `SetNotificationEnabled`: 设置时保存到 GUI 配置文件
- `GetNotificationEnabled`: 从 GUI 配置文件读取
- `SetDebugLogging`: 设置时保存到 GUI 配置文件
- `startup`: 启动时加载 GUI 设置并应用
...
1. **同步变量问题**: 由于 JavaScript 异步特性，`async/await` 无法在事件处理函数中同步返回结果。使用 `$effect` 监听状态变化，用同步变量 `rightClickDisabledSync` 来同步检查。

2. **配置文件路径**: `~/redc/gui_settings.json`

3. **默认值**: 
   - `disableRightClick: true` (默认禁用右键)
   - `notificationEnabled: false`
   - `debugEnabled: false`

4. **Wails WebView 限制**: Wails 的 WebView 可能在更底层处理了右键菜单，需要用 `capture` 阶段 (`addEventListener` 第三个参数为 `true`) 来确保先执行阻止逻辑。

### 2025-02-27 更新：代理配置持久化

#### 问题描述
代理配置保存在运行时环境变量中，每次重启应用后会丢失。

#### 解决方案
将代理配置也保存到 `~/redc/gui_settings.json` 中。
...
1. `SaveProxyConfig`: 保存时代理配置持久化到 GUI 设置
```go
...
3. `startup`: 启动时从 GUI 设置加载代理配置到环境变量
```go
...
1. **优先级**: GUI 设置 > 环境变量，设置保存后优先读取 GUI 设置
2. **默认值**: 空字符串表示未配置代理
3. **配置路径**: `~/redc/gui_settings.json`
4. **兼容性**: 如果配置文件没有代理字段（旧版本升级），不会报错，会使用环境变量作为回退
