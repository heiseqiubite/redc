...
从体系视角看，Redc 是 WgpSec Infra 的控制面。它向下消费 [redc-template](https://github.com/wgpsec/redc-template) 提供的场景、基础模板、专项模板和插件能力，向外通过 Terraform 管理多云资源，向内通过 [f8x](https://github.com/ffffffff0x/f8x) 完成目标主机上的软件安装与环境装配。对于用户而言，Redc 是统一入口，redc-template 是场景仓库，f8x 是软件装配引擎。
